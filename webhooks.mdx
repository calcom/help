...
1. Subscriber URL: The listener URL where the payload will be sent to when an event trigger is triggered. See [subscriber URL requirements](/webhooks#subscriber-url-requirements) for details on which URLs are accepted.
