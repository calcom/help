...
<Note>
A webhook created for a team will only trigger for team-level event types (e.g., Collective and Round Robin) belonging to that team.
However, in the case of Managed events, bookings are made on the child event types, not the parent team event. Because of this, team-level webhooks will not trigger for managed events unless one of the following is true:
