...
- You can check your available credit balance and charge credits programmatically using the API v2 endpoints below

### Checking available credits

Use the `GET /v2/credits/available` endpoint to check whether your account has credits remaining and to retrieve your current balance breakdown.

**Response fields:**

| Field | Description |
|---|---|
| `hasCredits` | Whether the account has any credits available |
| `balance.monthlyRemaining` | Credits remaining from the monthly allocation |
| `balance.additional` | Purchased credits available beyond the monthly allocation |
...
Use the `POST /v2/credits/charge` endpoint to charge credits after a completed AI agent interaction.
...
| `creditFor` | Yes | Usage type. Use `AI_AGENT` for AI agent interactions |
| `externalRef` | No | Unique reference string for idempotency to prevent double-charging |

**Response fields:**

| Field | Description |
|---|---|
| `charged` | Whether the charge was applied successfully |
| `teamId` | Team ID that was charged, if applicable |
| `remainingBalance.monthlyRemaining` | Monthly credits remaining after the charge |
| `remainingBalance.additional` | Purchased credits remaining after the charge |
...
<Tip>
  Use the `externalRef` field with a unique identifier for each interaction to prevent duplicate charges. If the same `externalRef` is sent twice, the second request will not charge credits again.
</Tip>