...
4. **Cross-TLD fuzzy domain matching** — If the previous steps found no match and fuzzy domain matching is enabled, Cal.com extracts the base company name from the attendee's email domain and compares it against account `Website` values across different top-level domains. For example, an attendee with an `@acme.co.uk` email matches an account whose website is `acme.com`, because both share the base domain `acme`. This works with all top-level domains including multi-level ones like `.co.uk`, `.com.au`, `.co.jp`, and more. Free email domains (e.g. gmail.com) and very short company names are automatically excluded to prevent false matches.
