...
When a booking is successfully created in Cal.com, users can be redirected to a custom success page by enabling `Redirect on booking`option in the Confirmation tab of the Booking experience section of the event type settings. This guide documents all the query parameters that are forwarded to your custom success URL when 'Forward parameters' option is enabled (which is the default).
