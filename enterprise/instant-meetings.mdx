...
#### Instant Meeting Created payload

The **Instant Meeting Created** payload carries the booker's form `responses` and the `connectAndJoinUrl` a host opens to accept and join the meeting:
...
```json
{
    ...
    "payload": {
        ...
        "responses": {
            ...
            "notes": "Need help choosing a plan",
            "guests": []
        },
        "connectAndJoinUrl": "https://app.cal.com/connect-and-join?token=EXAMPLE_TOKEN",
        "eventTypeId": 15,
...
<Note>
  The fields in `responses` depend on the booking questions configured on the event type, so a payload from your own event type can contain more or fewer fields than the example above. `customInputs` is empty unless the event type still uses legacy custom inputs.
</Note>

The **Instant Meeting Accepted** payload is documented in [Webhooks](/webhooks#instant-meeting-accepted).
