...
The portal's **Owners** blade only accepts users, so add the ownership with the [Azure CLI](https://learn.microsoft.com/cli/azure/install-azure-cli) or Microsoft Graph, using two different object IDs:
